...
cc -O1 -g -ffunction-sections -fdata-sections -Wall -Wextra \
   -Wno-unused-parameter -Werror -fsanitize=address,undefined --param asan-globals=0 \
   -I "$src/objs" -I "$src/src/core" -I "$src/src/os/unix" \
   -I "$src/src/event" -I "$src/src/event/modules" \
   -I "$src/src/event/quic" -I "$src/src/http" -I "$src/src/http/modules" \
   -I "$src/src/http/v2" -I "$src/src/http/v3" \
   "$tests/cache_policy.c" \
